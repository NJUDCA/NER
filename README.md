...
在训练Bert模型的过程中, 使用的是开源模型的参数`learning_rate=2e-5`, `batch_size=32`, 模型在验证集上的效果已经很好, 所以没有再调参数。但是加上BiLSTM层后, 在相同训练轮次(num_train_epochs=4.0)的情况下, 收敛速度减慢, 模型的训练效果不如单独的Bert, 可能是由于网络参数增加，需要更多的训练轮次（7.0）达到收敛效果。
